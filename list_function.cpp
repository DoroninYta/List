...
void list_ctor( LIST * list, int count_elements)
{
    ...
    list->data = (struct array *)calloc(count_elements, sizeof(struct array));
...
void list_push_back(LIST * list, Elem_t el)
{
    ...
    list->data[list->free_head].elem = el;
    if (list->tail != 0)
    {
        list->data[list->tail].next = list->free_head;
        list->data[list->free_head].prev = list->tail;
    }
    list->tail = list->free_head;
    list->free_head = list->data[list->free_head].next;
...
{   
    ...
    {   
        printf("|||DATA[%d] = %d    |   NEXT{%d] = %d   |   PREV[%d] = %d|||",
               i, list->data[i].elem, i, list->data[i].next, i, list->data[i].prev);
        if (list->data[i].elem == -429)
            printf(" POISON");
        printf("\n");
        i++;
    }  
